parse_overordnad_summering_kf_2026 <- function(raw) {
  ...
  parse_omrade <- function(obj, geografiniva,
  ...
  lan <- raw$helaLandet$lan |>
    purrr::map(\(x) {
      parse_omrade(
        x,
        geografiniva = "lan",
        lannamn = as_chr_na(x$namn),
        lankod = as_chr_na(x$lankod)
      )
    }) |>
    purrr::list_rbind()

  kommuner <- raw$helaLandet$lan |>
    purrr::map(\(lan) {
      lan$kommuner |>
        purrr::map(\(kommun) {
          parse_omrade(
            kommun,
            geografiniva = "kommun",
            lannamn = as_chr_na(lan$namn),
            lankod = as_chr_na(lan$lankod),
